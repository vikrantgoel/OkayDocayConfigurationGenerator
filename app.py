...
def remove_field_from_extracted_data():
    ...
    try:
        ...
        removed_count = 0
        for page_num, page_data in user_session.extracted_data.get('pages', {}).items():
            if 'fields' in page_data:
                original_count = len(page_data['fields'])
                page_data['fields'] = [f for f in page_data['fields'] if f.get('name') != field_name]
                removed_count += original_count - len(page_data['fields'])
        ...
        if removed_count > 0:
            ...
            return jsonify({
                'success': False,
                'error': f'Field "{field_name}" not found in server data'
            })
...
def generate_config_json():
    """Generate the final configuration JSON with single validation script and custom name"""
    try:
        user_session = get_user_session()
        data = request.get_json()
        ...
        if not config_name:
            config_name = user_session.extracted_data.get('pdf_name', 'Configuration').replace('.pdf', '')
        ...
        # Create safe filename - simple version
        safe_config_name = config_name.replace(' ', '_').replace('.', '_')
        # Remove any problematic characters
        safe_chars = []
        for char in safe_config_name:
            if char.isalnum() or char in '_-':
                safe_chars.append(char)
        safe_config_name = ''.join(safe_chars)

        if not safe_config_name:
            safe_config_name = 'Configuration'

        config_filename = f"{safe_config_name}_config_{int(time.time())}.json"
        ...
        zip_filename = f"{safe_config_name}_complete_package_{int(time.time())}.zip"
        ...
        with zipfile.ZipFile(zip_temp_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            # Add config JSON
            zipf.write(config_temp_path, 'config.json')
...
Run script: python validation_script.py
"""
            zipf.writestr('README.md', simple_readme)
        ...
        if upload_to_github_flag and GITHUB_TOKEN:
            try:
                ...
                if not repo_result['success']:
                    return jsonify(
                        {'success': False, 'error': f"GitHub repository setup failed: {repo_result['error']}"})

                # Upload ZIP to GitHub
                upload_result = upload_zip_to_github(zip_temp_path, safe_config_name, 'custom_config')
                ...
                if upload_result['success']:
                    return jsonify({
                        ...
                        'config_name': config_name,
                        'message': f"Configuration uploaded to GitHub successfully!"
                    })
                else:
